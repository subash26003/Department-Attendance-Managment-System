...
const AttendanceDashBoard = () => {
  ...
  const fetchAttendanceRecords = async () => {
    ...
    try {
      ...
      let query = `/dashboard?year=${year}&subjectCode=${subjectCode}`
  ...
  const renderSuccessView = () => {
    return (
      <div className=" p-1 md:p-4 bg-gray-50 min-h-full">
          ...
          {viewMode === "student" && (
            <StudentAttendanceHistory data={data.studentSummary} />
          )}
      ...
      <Heading title={" Attendance Dashboard"} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 bg-white p-4 rounded-lg shadow">
          ...
          <div className="flex items-end gap-2">
            ...
            <button onClick={fetchAttendanceRecords} className="w-full p-2 bg-gray-200 hover:bg-gray-300 rounded-md">
              Apply
            </button>
          </div>
        </div>
