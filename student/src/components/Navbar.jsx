...
const Navbar = () => {
  ...
  return (
    <div className='bg-white px-5 md:px-10 py-3 flex items-center justify-between lg:justify-start  font-medium shadow-lg'>
        <Link to="/"><img src={Logo} className='w-14 md:w-24 self-start' alt="logo"/></Link>
        <div className='lg:flex items-center self-center w-full  lg:m-auto gap-5 hidden text-gray-800'>
            <div className='lg:flex justify-center w-full m-auto  gap-5'>
                ...
                <NavLink to='/request' className='navlink text-xl border-blue-500 '>Request</NavLink>
            </div>
            <div className='group relative'>
                ...
                <div className='group-hover:block hidden absolute right-0 text-xl card-shadow bg-white rounded-lg w-52'>
                    <p onClick={() => naviagte("/profile")} className=' p-2  hover:bg-gray-100 text-gray-700 cursor-pointer'>Profile</p>
                    <p onClick={handleLogout} className='border-t p-2 hover:bg-gray-100 text-gray-700 cursor-pointer'>Logout</p>
                </div>
            </div>
        </div>
        ...
        <div className={`'hidden lg:w-0 absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all ${showMenu ? 'w-full' : 'w-0'} '`}>
            <div className='flex flex-col text-gray-600'>
                ...
                <NavLink onClick={() => setShowMenu(false)} to='/request' className="nav-links py-4 px-4 border-b">Request</NavLink>
                {/* <NavLink onClick={() => setShowMenu(false)} to='/myclass' className="nav-links py-4 px-4 border-b">Myclass</NavLink> */}
                <NavLink onClick={() => setShowMenu(false)} to='/profile' className="nav-links py-4 px-4 border-b">PROFILE</NavLink>
                <p onClick={() => {setShowMenu(false); handleLogout()}}  className="nav-links py-4 px-4 border-b cursor-pointer">LOGOUT</p>
            </div>
        </div>
    </div>
  )
